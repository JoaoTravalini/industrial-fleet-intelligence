"""Host-side wrapper for running Spark Bronze ingestion inside Docker."""
...
BRONZE_RUNNER = "/workspace/scripts/run_spark_bronze.py"
...
def build_spark_submit_command(config: bronze_ingestion.SparkBronzeConfig) -> list[str]:
    return [
        "docker",
        "compose",
        "exec",
...
def run_spark_bronze(timeout: int = DEFAULT_TIMEOUT_SECONDS) -> CommandResult:
    config = bronze_ingestion.load_spark_config()
    command = build_spark_submit_command(config)
